...
export async function createPage(req, res) {
    try {
        ...
        const userPages = await Page.find({ user: req.user.id });
        if (user.plan === "free" && userPages.length >= 1) {
            return res.status(403).json({
                error: "Free plan allows only 1 page. Upgrade to Pro for more.",
            });
        }
...
export async function trackPageVisit(req, res) {
    try {
        ...
        return res.status(200).json({ message: "Page visit tracked!" });
    } catch (error) {
        console.error("Error tracking page visit:", error);
        return res.status(500).json({ message: "Error tracking page visit", error });
    }
}